...
export function generateWorkout(args) {
    ...
    let sets = SCHEMES[scheme].ratio
        .reduce((acc, curr, index) => {
            //make this compound and exercise muscles -> array of objects and destructure in loop
            return [
                ...acc,
        ...
        .reduce((acc, curr, index) => {
            ...
            return [
                ...
                {
                    setType: curr,
                    musclesGroup: musclesGroupToUse,
                },
            ];
        }, []);
    ...
    const genWOD = sets.map(({ setType, musclesGroup }) => {
        const data =
        ...
        const filteredObj = Object.keys(data).reduce((acc, curr) => {
            if (
                ...
                !data[curr].muscles.includes(musclesGroup)
            ) {
